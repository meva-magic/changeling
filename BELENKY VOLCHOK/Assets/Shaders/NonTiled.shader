    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings input) : SV_Target
            {
                ...
                #ifndef STRETCH
                // Fit mode: preserve aspect ratio by adjusting UVs
                float2 textureSize = float2(_MainTex_TexelSize.z, _MainTex_TexelSize.w);
                
                // Get object scale to determine quad proportions
                ...
                // Center the texture and add letterboxing
                if (quadAspect > textureAspect)
                {
                    // Quad is wider - letterbox left/right
                    float scale = textureAspect / quadAspect;
                    uv.x = (uv.x - 0.5) / scale + 0.5;
                }
                else
                {
                    // Quad is taller - letterbox top/bottom
                ...
                // Sample texture (clamped to prevent tiling)
